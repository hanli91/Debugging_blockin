...
def iterative_topK_debug_blocker(ltable, rtable, candidate_set, pred_list_size=100, field_corres_list=None):
    ...
    token_to_id_dict = {}
    id_to_token_dict = {}
    '''We only need building global order for one table.'''
    build_global_token_order(lrecord_list, token_freq_dict, token_to_id_dict, id_to_token_dict)
    #build_global_token_order(rrecord_list, order_dict)
    #print lrecord_list

    logging.info('\nSorting tables')
    # Sort each record by the token global order
    lrecord_token_list, lrecord_field_list = sort_record_tokens_by_global_order(
        lrecord_list, token_freq_dict, token_to_id_dict)
    rrecord_token_list, rrecord_field_list = sort_record_tokens_by_global_order(
        rrecord_list, token_freq_dict, token_to_id_dict)
    ...
    topK_heap, compared_dict = run_iteration(lrecord_token_list, lrecord_field_list,
                                             rrecord_token_list, rrecord_field_list,
                                             {}, 0, None, pred_list_size, -1, field_index_list)
    ...
    for field_index in field_index_list:
        ...
        run_iteration(lrecord_token_list, lrecord_field_list,
                      rrecord_token_list, rrecord_field_list,
                      compared_dict, 0, None, pred_list_size, field_index, remained_fields)

    return None


def run_iteration(lrecord_token_list, lrecord_field_list, rrecord_token_list, rrecord_field_list,
    ...
    lrecord_length_list = calc_record_length(lrecord_field_list, remained_fields)
    rrecord_length_list = calc_record_length(rrecord_field_list, remained_fields)

    logging.info('\nGenerating prefix events')
    #Generate prefix events
    lprefix_events = []
    rprefix_events = []
    generate_prefix_events(lrecord_field_list, lrecord_length_list, lprefix_events, remained_fields, lower_bound)
    generate_prefix_events(rrecord_field_list, rrecord_length_list, rprefix_events, remained_fields, lower_bound)
...
def calc_record_length(record_field_list, remained_fields):
    record_length_list = []
    for i in range(len(record_field_list)):
        full_length = len(record_field_list[i])
        actual_length = 0
        for j in range(full_length):
            if record_field_list[i][j] in remained_fields:
                actual_length += 1
...
def generate_prefix_events(record_field_list, record_length_list, prefix_events, remained_fields, lower_bound):
    for i in range(len(record_field_list)):
        ...
        for j in range(full_length):
            if record_field_list[i][j] in remained_fields :
                ...
                if threshold >= lower_bound:
                    hq.heappush(prefix_events, (-1.0 * threshold, i, j, count, record_field_list[i][j]))
                count += 1
...
def sort_record_tokens_by_global_order(record_list, token_freq_dict, token_to_id_dict):
    record_token_list = []
    record_field_list = []
    for i in range(len(record_list)):
        tmp_list = []
        for j in range(len(record_list[i])):
            if record_list[i][j][0] in token_freq_dict:
                tmp_list.append(record_list[i][j])

        tmp_list = sorted(tmp_list, key=lambda x: token_freq_dict[x[0]])
        token_list = []
        field_list = []
        for j in range(len(tmp_list)):
            token_list.append(token_to_id_dict[tmp_list[j][0]])
            field_list.append(tmp_list[j][1])
        record_token_list.append(token_list)
        record_field_list.append(field_list)
    return record_token_list, record_field_list


def build_global_token_order(record_list, token_freq_dict, token_to_id_dict, id_to_token_dict):
    for record in record_list:
        for token_tuple in record:
            token = token_tuple[0]
            if token_freq_dict.has_key(token):
                token_freq_dict[token] = token_freq_dict[token] + 1
            else:
                token_freq_dict[token] = 1

    count = 0
    for key in token_freq_dict:
        token_to_id_dict[key] = count
        id_to_token_dict[count] = key
        count += 1
